...
export function Layout() {
  ...
  return (
    <div className="h-screen flex bg-[#F4F7FC] font-sans">
      ...
      <aside
        className={`bg-[#0E1729] text-white transition-all duration-300 flex flex-col shrink-0 ${sidebarOpen ? 'w-[259px]' : 'w-0'
          ...
          {allowedNavItems.map((item) => {
            ...
            return (
              <Link
                key={item.path}
                to={item.path}
                className={`flex items-center gap-[14px] px-[26px] py-[13px] mx-2 rounded-[7px] transition-colors text-[14px] font-normal group focus-visible:ring-2 focus-visible:ring-site-orange focus-visible:outline-none ${isActive
              ...
              <div className="max-h-[280px] overflow-y-auto custom-scrollbar p-1">
                {MOCK_USERS.map((user) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 p-2 rounded-lg transition-colors text-left ${currentUser.id === user.id ? 'bg-[#262027] border border-[#A04F22]/30' : 'hover:bg-[#2d3b55] border border-transparent'
                      }`}
                  >
